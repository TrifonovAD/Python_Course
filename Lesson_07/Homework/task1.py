...
def rithm_func(puh_str):
    vowels = "аеёиоуыэюя"
    vowels_count=[]
    for word in puh_str:
        vowels_count.append(sum(1 for char in word if char.lower() in vowels))
    return vowels_count

my_str = input().split()
result = rithm_func(my_str)
print("Парам пам-пам" if len(set(result)) == 1 and result[0] != 0 else "Пам парам")